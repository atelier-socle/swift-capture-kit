...
Comprehensive mock server supporting 5 modes + directory serving:
  ...
  --dir   — Serve HLS files from a local directory (live Safari-compatible)
...
Usage:
  ...
  python3 mock-hls-server.py --dir /path/to/hls-output/ --port 8080
"""
...
class HLSHandler(BaseHTTPRequestHandler):
    ...
    live_state = None
    dir_root = None         # For --dir mode: absolute path to serve from
    _request_count = 0
    ...
    def _serve_dir_file(self, url_path):
        """Serve a file from dir_root with proper HLS headers."""
        # Normalize and resolve the path, prevent directory traversal
        clean = os.path.normpath(url_path.lstrip("/"))
        file_path = os.path.join(self.dir_root, clean)
        file_path = os.path.realpath(file_path)

        if not file_path.startswith(os.path.realpath(self.dir_root)):
            self.send_error(403, "Forbidden")
            log(f"GET {self.path} → 403 (path traversal blocked)", Color.RED)
            return

        if not os.path.isfile(file_path):
            self.send_error(404, "Not Found")
            log(f"GET {self.path} → 404 ({file_path})", Color.RED)
            return
        ...
        try:
            ...
            log(f"GET {self.path} → 500 ({e})", Color.RED)
            return
        ...
        # Playlists: always fresh (no caching for live polling)
        if ext == ".m3u8":
            self.send_header("Cache-Control", "no-cache, no-store")
        # Segments and init: immutable
        elif ext in (".m4s", ".mp4", ".ts"):
            self.send_header("Cache-Control", "max-age=3600")

        self.end_headers()
        self.wfile.write(data)

        self.stats.record_request(self.path, sent_len=len(data))
        log(f"GET {self.path} → {len(data)} bytes ({content_type})", Color.GREEN)

    def do_HEAD(self):
        """Handle HEAD requests (Safari sends these for HLS probing)."""
        if self.dir_root is not None:
            parsed = urlparse(self.path)
            clean = os.path.normpath(parsed.path.lstrip("/"))
            file_path = os.path.realpath(
                os.path.join(self.dir_root, clean))
            if not file_path.startswith(
                os.path.realpath(self.dir_root)
            ...
            self.end_headers()
            return
        # For non-dir modes, just delegate to GET (body ignored by HTTP)
        self.do_GET()
    ...
    def do_GET(self):
        ...
        # ── Dir mode: serve files from disk ──
        if self.dir_root is not None:
            self._serve_dir_file(path)
            return
...
def print_endpoints(port, mode):
    ...
    if mode in ("serve", "live"):
        ...
        dir_root = HLSHandler.dir_root or "."
        print(f"  {Color.YELLOW}Directory File Server{Color.RESET}")
        print(f"    Serving: {Color.GREEN}{dir_root}{Color.RESET}")
        print(f"    .m3u8 → Cache-Control: no-cache, no-store")
        print(f"    .m4s/.mp4/.ts → Cache-Control: max-age=3600")
        print(f"    CORS: Access-Control-Allow-Origin: *")
        print()
        # List top-level subdirectories as hints
        try:
            entries = sorted(os.listdir(dir_root))
            dirs = [e for e in entries if os.path.isdir(os.path.join(dir_root, e))]
            m3u8s = [e for e in entries if e.endswith(".m3u8")]
            if dirs or m3u8s:
                print(f"  {Color.YELLOW}Available content:{Color.RESET}")
                for d in dirs[:10]:
                    print(f"    {Color.DIM}{base}{Color.RESET}/{d}/")
                for m in m3u8s[:5]:
                    print(f"    {Color.DIM}{base}{Color.RESET}/{m}")
                print()
        except OSError:
            pass
...
def run_server(args):
    ...
    HLSHandler.dir_root = getattr(args, "dir", None)
    ...
    extra_lines = []
    if mode == "dir":
        extra_lines.append(f"Dir: {Color.GREEN}{HLSHandler.dir_root}{Color.RESET}")
    if fail:
    ...
    # Main server — ThreadingHTTPServer handles concurrent Safari requests
    ServerClass = ThreadingHTTPServer if mode == "dir" else HTTPServer
    ServerClass.allow_reuse_address = True
    server = ServerClass(("0.0.0.0", port), HLSHandler)
    server.timeout = 1
...
{Color.BOLD}Examples:{Color.RESET}
  ...
  {Color.YELLOW}Directory file server{Color.RESET} — serve HLS output from disk:
    python3 mock-hls-server.py --dir /path/to/hls-output/
    python3 mock-hls-server.py --dir ~/Documents/hls-2026-03-15/ --port 9090
...
"""
    ...
    parser.add_argument(
        ...
        help="Serve files from a local directory (e.g. --dir /path/to/hls/). "
             "Overrides --mode. Playlists get Cache-Control: no-cache, "
             "segments get max-age=3600.",
    )
...
def main():
    ...
    if args.dir:
        ...
        if not os.path.isdir(dir_path):
            parser.error(f"--dir path does not exist: {dir_path}")
        args.mode = "dir"
        args.dir = dir_path
