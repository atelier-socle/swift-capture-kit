...
/// Delivers raw encoded buffers via closures or AsyncStream.
///
/// Use for custom processing, analysis, or forwarding to external systems.
///
/// ```swift
/// let callback = CallbackOutput(
///     audioHandler: { buffer in print("Audio: \(buffer.data.count) bytes") },
///     videoHandler: { frame in print("Video: \(frame.data.count) bytes") }
/// )
/// ```
@available(macOS 14.0, iOS 17.0, visionOS 1.0, *)
public actor CallbackOutput: CaptureOutput {
    /// A unique identifier for this output.
    public let outputID: String

    /// A human-readable display name for this output.
    public let displayName: String = "Callback Output"

    /// The type of this output destination.
    public nonisolated let outputType: CaptureOutputType = .callback

    /// The current lifecycle state of this output.
    public private(set) var state: CaptureOutputState = .idle

    /// Number of audio buffers delivered.
    public private(set) var audioDeliveryCount: Int64 = 0

    /// Number of video frames delivered.
    public private(set) var videoDeliveryCount: Int64 = 0
    ...
    /// Creates a new callback output with optional audio and video handlers.
    ///
    /// - Parameters:
    ///   - audioHandler: A closure called for each encoded audio buffer.
    ///   - videoHandler: A closure called for each encoded video frame.
    public init(
        audioHandler: (@Sendable (EncodedAudioBuffer) async -> Void)? = nil,
        videoHandler: (@Sendable (EncodedVideoFrame) async -> Void)? = nil
    ) {
        self.outputID = "callback-\(UUID().uuidString.prefix(8))"
